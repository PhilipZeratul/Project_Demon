...
{
    [BurstCompile]
    struct CollisionJob : IJobForEach<Translation, PhysicsObject>
    {
        ...
        public void Execute(ref Translation position, ref PhysicsObject p)
        {
            p.rx += p.dx;
            p.ry += p.dy;
            p.dx *= 0.96f;
    ...
    private EntityQuery objGroup;


    protected override void OnCreateManager()
    {
        mapGroup = GetEntityQuery(typeof(Dungeon));
        objGroup = GetEntityQuery(typeof(PhysicsObject));
    }

    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        var mapEntityArray = mapGroup.ToEntityArray(Allocator.TempJob);
        Dungeon map = EntityManager.GetSharedComponentData<Dungeon>(mapEntityArray[0]);
